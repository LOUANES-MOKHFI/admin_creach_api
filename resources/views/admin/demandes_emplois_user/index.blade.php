...
@section('content')
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الرئيسية</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ طلبات العمل</span>
            </div>
        </div>
                        ...
                        <table id="example1" class="table key-buttons text-md-nowrap" data-page-length='50'style="text-align: center">
                            ...
                            @isset($demandes)
                                @foreach($demandes as $key=>$demande)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$demande->emploi_id == 31 ? $demande->other_emploi : $demande->emploi->name}} </td>
                                        <td>{{$demande->user ? $demande->user->name : ''}}</td>
                                        <td>{{$demande->degre_etude}}</td>
                                        <td>{{$demande->experience}}</td>
                                        <td>{{$demande->phone}}</td>
                                        <td>{{$demande->wilaya ? $demande->wilaya->name : ''}} - {{$demande->commune ? $demande->commune->name : ''}}</td>
                                        <td>
                                            <a href="{{route('admin.demandes_emplois_user.destroy',$demande->id)}}"class="btn btn-danger waves-effect waves-light" title="حذف">
