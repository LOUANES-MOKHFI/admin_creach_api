...
@section('content')
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الرئيسية</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ مجالات الاستشارات</span>
            </div>
        </div>
        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                        <a href="{{route('admin.settings.domaines_conseils.create')}}" class="modal-effect btn btn-sm btn-primary" style="color:white"><i
                                class="fas fa-plus"></i>&nbsp; أضف مجال
                        </a>
                            ...
                            @isset($domaines)
                                @foreach($domaines as $key=>$domaine)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$domaine->name}}</td>
                                        <td>{{$domaine->admin->name}}</td>
                                        <td>
                                        <a href="{{route('admin.settings.domaines_conseils.edit',$domaine->uuid)}}"class="btn btn-warning waves-effect waves-light" title="{{__('admin/patients.edit')}}">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <a href="{{route('admin.settings.domaines_conseils.delete',$domaine->uuid)}}" class="btn btn-danger waves-effect waves-light" title="{{__('admin/patients.delete')}}">
