...
@section('content')
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">الرئيسية</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ الدليل البيداغوجي</span>
            </div>
        </div>
        ...
        <div class="col-xl-12">
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                        <a href="{{route('admin.guide_pedagogique.create')}}" class="modal-effect btn btn-sm btn-primary" style="color:white"><i
                                class="fas fa-plus"></i>&nbsp; أضف دليل بيداغوجي
                        </a>
                            ...
                            @isset($guides)
                                @foreach($guides as $key=>$guide)
                                    <tr>
                                        <td>{{$key+1}}</td>
                                        <td>{{$guide->name}}</td>
                                        <td><a target="_blank" href="{{asset('files/guide_pedagogique/'.$guide->pdf_file)}}" class="btn btn-success btn-sm"><i class="fa fa-download"></i></a> </td>
                                        <td>
                                        <a href="{{route('admin.guide_pedagogique.edit',$guide->uuid)}}"class="btn btn-warning waves-effect waves-light" title="تعديل">
                                            <i class="fa fa-edit"></i>
                                        </a>
                                        <a href="{{route('admin.guide_pedagogique.delete',$guide->uuid)}}" class="btn btn-danger waves-effect waves-light" title="حذف">
